...
const destroy = async (req, res) => {
  try {
    ...
    await pool
      .request()
      .input("WarehouseID", WarehouseID)
      .query(WAREHOUSE_QUERY.DELETE);

    return res.status(200).json({
      message: WAREHOUSE_MESSAGES.DELETED,
    });
  } catch (error) {
    return res.status(400).json({
      message: MESSAGES.ERROR,
...
const getByBranch = async (req, res) => {
  try {
    ...
    const { recordset: rows } = await pool
      .request()
      .input("BranchID", BranchID)
      .query(WAREHOUSE_QUERY.BY_BRANCH_ID);
    return res.status(200).json(rows);
  } catch (error) {
    return res.status(400).json({
      message: MESSAGES.ERROR,
...
export default { create, getAll, update, destroy, getByBranch };
